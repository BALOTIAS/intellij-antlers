...
- **`.antlers.xml` views** (sitemaps, RSS/Atom feeds) are now recognized — Antlers layered over **XML**
  ...
  elements (`{{ if }}<url>…</url>{{ /if }}`), Antlers in tag-attribute position
  (`<urlset {{ yield:namespace }}>`), and `{var}` interpolations in params (`from="{handle}"`) don't
  trigger false well-formedness errors.

### Fixed

- A `{var}` **string interpolation whose variable shares a pair tag's name** (`from="{taxonomy}"` inside
  `{{ taxonomy … }}`) no longer makes the enclosing tag falsely report *"is never closed"* — the injected
  interpolation fragment, a tiny variable expression, is no longer balance-checked. (Affected `.antlers.html`
  too.)
...
[Unreleased]: https://github.com/BALOTIAS/intellij-antlers/compare/1.0.9...HEAD
[1.0.9]: https://github.com/BALOTIAS/intellij-antlers/compare/1.0.8...1.0.9
